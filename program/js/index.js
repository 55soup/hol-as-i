...
function initialize() { // google 지도
    ...
    promise.then(setTimeout(function(){ // 비동기호출
        ...
        let map = new google.maps.Map(document.getElementById("map"), // div의 id과 값이 같아야 함. "map-canvas"
                            mapOptions);

        let size_x = 40; // 마커로 사용할 이미지의 가로 크기
        let size_y = 40; // 마커로 사용할 이미지의 세로 크기

        // 마커로 사용할 이미지 주소
        let image = new google.maps.MarkerImage( '주소를 입력하세요',
        ...
        for (let i = 0; i<apis.length; i++){
            let address = apis[i]["PLACE_ADRES1"]; // DB에서 주소 가져와서 검색하거나 왼쪽과 같이 주소를 바로 코딩.
            let program_name=apis[i]["PARTCPTN_SJ"];
            let marker = null;
            ...
            if (status == google.maps.GeocoderStatus.OK) {
                ...
                marker = new google.maps.Marker({
                    ...
                    title: program_name, // 마커에 마우스 포인트를 갖다댔을 때 뜨는 타이틀
